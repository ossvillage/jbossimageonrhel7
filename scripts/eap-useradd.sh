...
echo "Configuring EAP managment user" >> /tmp/install.progress.txt
$EAP_HOME/bin/add-user.sh -u $EAP_USER -p $EAP_PASSWORD -g 'guest,mgmtgroup'

echo "Start EAP 7.2" >> /tmp/install.progress.txt
systemctl restart eap7-standalone.service 

# Seeing a race condition timing error so sleep to deplay
sleep 20

echo "ALL DONE!" >> /tmp/install.progress.txt
/bin/date +%H:%M:%S >> /tmp/install.progress.txt
